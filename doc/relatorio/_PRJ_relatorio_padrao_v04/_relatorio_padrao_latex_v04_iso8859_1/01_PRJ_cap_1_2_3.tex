...
Neste trabalho, a anotação de eventos sonoros em vídeo refere-se à análise do áudio extraído de um vídeo previamente gravado, com o objetivo de recolher informações relativas a possíveis batidas de bola. O processamento sobre o áudio será realizado em modo \textit{offline}.

Este trabalho destina-se à análise da componente desportiva de padel (\cite{courel2019exploring}), no sentido de fornecer a atletas e/ou treinadores um complemento no decurso do treino.

Uma figura referente ao campo onde este desporto é praticado e à raquete e bola utilizadas encontra-se disponível no Capítulo \ref{ch:umDetalheAdicional}, na qual se observam as dimensões do campo onde esta modalidade desportiva é realizada. O desporto de padel pode ser praticado em ambiente interior (\textit{indoor}) ou exterior (\textit{outdoor}). No entanto, este trabalho analisa apenas vídeos em ambiente \textit{indoor}.
...
\begin{itemize}
\item
Disponibilizar de um \textit{dataset} construído de raiz;
...
Um caso particular da validação é a técnica de validação cruzada \cite{mohammed_cv_2020}. Esta técnica divide os dados em K grupos (\textit{folds}) e ao longo de K iterações treina modelos diferentes onde K-1 \textit{folds} são utilizados para treino e um \textit{fold} é utilizado para validação.
...
O funcionamento do sistema pode ser representado como se mostra na figura \ref{fig:systemscheme}. O sistema desenvolvido reconhece batidas de bola, permite auxiliar no processo de construção do dataset e, consequentemente no processo de melhoria de desempenho do classificador.
