...
Esta sub-secção faz uma descrição da aborgem adotada para construir a aplicação que deve mostrar os resultados obtidos pelo classificador.

\paragraph{\textbf{Estrutura da Aplicação}} \hspace{0pt} \\

\noindent
A aplicação \textit{web} será desenvolvida utilizando a linguagem PHP. Esta linguagem é bastante útil porque é executada do lado do servidor e permite de forma flexível criar interação com o utilizador através de páginas HTML dinâmicas. No lado do cliente será utilizada a linguagem Javascript. (REFERIR QUE SÃO FEITOS PEDIDOS GET EPOST AO SERVIDOR) e que são realizados pedidos AJAX e que o AJAX é útil para não ter de fazer refresh na página toda, só pequenas porções da página. 
...
Para efeitos de seimplificação, a página 1 terá um lista de vídeos que já foram classificados. Pelo que o utilizador deve escolher um vídeo. Ao selecionar um vídeo é redirecionado para a página 2, onde pode selecionar os vários eventos (à esquerda) e visualizá-los (à direita).

\begin{figure}[h]
   \centering
   \includegraphics[width=12cm]{./mockup_pagina_web_1}
