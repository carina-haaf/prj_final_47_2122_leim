...
def construct(nr_groups, nr_samples_per_group, nr_shifted_samples, features_file,
              file_rows, video, sample_rate=44100):

    total_ball_hits = 0
    total_non_ball_hits = 0

    files = np.array(list(os.listdir("../test_videos/")))
    print(files)
    audio_path = "../test_audios/AUDIO.wav"
    video.audio.write_audiofile(audio_path, fps=sample_rate)
    y, sr = librosa.load(audio_path, sr=None)

    nr_ball_hits, nr_non_ball_hits = get_data_features(y, 58, nr_groups, nr_samples_per_group,
